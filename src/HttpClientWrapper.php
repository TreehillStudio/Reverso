...
use const CURLE_URL_MALFORMAT_USER;
...
class HttpClientWrapper
{
    ...
    public function sendRequestWithBackoff(string $method, string $url, ?array $options = []): array
    {
        ...
        $this->logInfo("Request to Reverso API $method $url");
        ...
        if ($exception !== null) {
            ...
            $this->logInfo("Reverso API response $method $url $statusCode");
    ...
    /**
     * Sends an HTTP request. Note that in the case of a custom HTTP client, some of these options are
     ...
     */
    private function createHttpRequest(string $method, string $uri, array $headers, StreamInterface $body): RequestInterface
    {
    ...
    /**
     * Sends an HTTP request using the custom HTTP client.
     * @param string $method HTTP method to use.
     * @param string $url Absolute URL to query.
     * @param array $headers Array of headers to include in request.
     * @param array $params Array of parameters to include in body.
    ...
    ): array
    {
        ...
        if ($filePath !== null) {
            ...
            if (!isset($params['body'])) {
                ...
                $curlOptions[CURLOPT_POSTFIELDS] = $params['body'];
            }
        }
